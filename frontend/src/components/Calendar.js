...
const MobileCalendar = ({ onTimeSlotSelect }) => {
  ...
  const loadAvailableSlots = async (date) => {
    ...
    try {
      ...
      const response = await reservationService.getAvailableSlots(dateStr);

      // Handle different API response formats
      const slots = response.availableSlots || response.data || response || [];
          ...
          <div className="animate-fadeIn">
            ...
            {error && (
              <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-xl flex items-center space-x-2">
                ...
                <button
                  onClick={() =>
                    selectedDate && loadAvailableSlots(selectedDate)
                  }
                ...
                {availableSlots.map((slot, index) => (
                  <button
                    key={
                      typeof slot === "string"
                        ? slot
                        : slot.time || slot.startTime || index
                    }
                    onClick={() =>
                      handleSlotSelect(
                        typeof slot === "string"
                  ...
                  >
                    <div className="text-center">
                      <Clock className="h-6 w-6 mx-auto mb-2 text-blue-600 group-hover:text-white" />
